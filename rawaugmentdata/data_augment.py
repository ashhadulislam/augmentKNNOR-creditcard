...
class DataAugment:
	...
	def augment(self,**params):
		...
		index_to_use=math.floor(dist_percent*len(kth_distances_sorted))
		# print("Index is ",index_to_use)
		if index_to_use==0:
			print("Please increase your distance threshold, index value is ",index_to_use) 
			return [],[],[]
		threshold_dist=kth_distances_sorted[index_to_use]
		...
		while N>0:
			...
			for i in range(len(x)):
				...
				# print("Comparing",kth_distances[i],threshold_dist)                
				...
				for kk in range(Q):
					...
					# if posit[0]<=len(x) or (randmx<10**(-4)):					
						...
						count_not_usable+=1
					#print("Not usable ",count_not_usable,len(x))
					if count_not_usable>=len(x)-1:
						print("All new points getting classified as other class. Reduce k or increase dist_max")
						return [],[],[]
